...
query <- "select * from (
  ...
  left join analysisncr.at_rapid_pathway@cas2407 rp on pat.nhsnumber = rp.nhsnumber --added 16/09/2024, only patients with a diagnosis in RCRD
...
REDO THIS WITH PATHWAY NHS NUMBER 
...
###### REMOVING DUPLICATES, INVALID OFFERED CODES, AND CALCULATING INITIAL NUMBERS #######
...
hna_pcsp_data <- hna_pcsp_data |>
  ...
  filter(offered_code != "") |> #removing weird codes
  filter(nchar(offered_code) <3) |> #removing any remaining invalid offered codes
