...
query <- "select * from (
  ...
  and not exists(--excluding patients with a previous cancer diagnosis
        select 1 
        from av2021.at_tumour_england@casref01 tum_a
        where tum.patientid = tum_a.patientid
        and tum_a.diagnosisyear < 2021
        and tum_a.diagnosisyear > 2000 -- limiting to 2001+ diagnoses as per counting cases SOP
        and tum_a.cascade_inci_flag = 1
        and tum_a.dedup_flag = 1
        and substr(tum_a.site_icd10_o2_3char, 1, 1) = 'C' 
        and tum_a.site_icd10_o2_3char <> 'C44'
        and tum.tumourid <> tum_a.tumourid)
